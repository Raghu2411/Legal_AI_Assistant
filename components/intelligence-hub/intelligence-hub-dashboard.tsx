...
import { ArrowLeft, MessageSquare, FileText, ShieldCheck, User } from 'lucide-react';
...
import { HubClientSelector } from './client-selector';
...
interface IntelligenceHubDashboardProps {
...
export default function IntelligenceHubDashboard({
  initialClients,
  initialClientId,
}: IntelligenceHubDashboardProps) {
  const [clientId, setClientId] = useState<string | undefined>(initialClientId);
  const [isVendorOnly, setIsVendorOnly] = useState(false);

  if (!clientId) {
    return <HubClientSelector clients={initialClients} onSelect={setClientId} />;
  }

  const selectedClient = initialClients.find(c => c.id === clientId);
    ...
    <div className="p-8 flex flex-col gap-8 max-w-6xl mx-auto h-[calc(100vh-4rem)]">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          <Button variant="ghost" size="sm" onClick={() => setClientId(undefined)} className="gap-2">
        ...
        <div className="flex items-center gap-2 bg-accent/50 px-4 py-2 rounded-full border border-primary/10">
          <User className="h-4 w-4 text-primary" />
          <span className="text-sm font-semibold">{clientName}</span>
          <Badge variant="secondary" className="text-[10px] uppercase ml-2">
            {selectedClient?.auto_case_id}
          </Badge>
        </div>
      </div>
            ...
            <Card className="h-full flex flex-col">
              <CardHeader className="pb-3 flex flex-row items-center justify-between space-y-0">
                <div>
                  ...
                  <CardDescription>
                    Conversational AI grounded in {clientName}&apos;s document vault.
                  </CardDescription>
                </div>
                {isVendorOnly && (
          ...
          <TabsContent value="briefing" className="h-full m-0">
            <Card className="h-full flex flex-col">
              <CardHeader>
                ...
                <CardDescription>
                  Adaptive document summaries based on {clientName}&apos;s document types.
                </CardDescription>
              </CardHeader>
              <CardContent className="flex-1 overflow-hidden p-0 border-t">
